...
function animateJar(visual,p){const img=visual?.querySelector('img');if(!img)return;const t=transformFor(p);motion.introScale=spring(motion.introScale,t.scale,.16);motion.introY=spring(motion.introY,t.y,.14);img.style.setProperty('transform',`translate3d(-50%,calc(-50% + ${motion.introY}vh),0) scale(${motion.introScale})`,'important')}
