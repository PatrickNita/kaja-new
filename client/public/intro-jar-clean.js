...
const catalogueLayout=[
  {x:16,y:18,s:.98,r:-28,dx:-118,dy:-74,dr:-72},
  {x:34,y:11,s:.82,r:18,dx:-62,dy:-116,dr:54},
  {x:62,y:13,s:.92,r:-12,dx:58,dy:-112,dr:-56},
  {x:84,y:28,s:1.02,r:30,dx:132,dy:-55,dr:70},
  {x:86,y:58,s:.84,r:-40,dx:140,dy:48,dr:-66},
  {x:66,y:83,s:.92,r:14,dx:78,dy:128,dr:58},
  {x:39,y:86,s:.86,r:-18,dx:-48,dy:132,dr:-56},
  {x:15,y:65,s:.96,r:35,dx:-132,dy:62,dr:72},
  {x:20,y:42,s:.80,r:-50,dx:-122,dy:-12,dr:-70},
  {x:52,y:54,s:.90,r:46,dx:18,dy:50,dr:68}
];
...
function animateCatalogueItems(scene,p){if(!scene)return;motion.catalogueProgress=spring(motion.catalogueProgress,p,.16);const eased=smooth(motion.catalogueProgress);const middle=scene.querySelector('.catalogue-mixology-middle');if(middle){middle.style.left='50%';middle.style.top='50%';middle.style.setProperty('transform',`translate3d(-50%,-50%,0) rotate(${eased*4}deg) scale(${1+eased*.035})`,'important')}scene.querySelectorAll('.catalogue-mixology-item').forEach((item)=>{const x=Number(item.dataset.x);const y=Number(item.dataset.y);const s=Number(item.dataset.s);const r=Number(item.dataset.r);const dx=Number(item.dataset.dx);const dy=Number(item.dataset.dy);const dr=Number(item.dataset.dr);const left=50+(x-50)*eased;const top=50+(y-50)*eased;const pop=0.62+eased*.38;item.style.left=`${left}%`;item.style.top=`${top}%`;item.style.setProperty('transform',`translate3d(calc(-50% + ${dx*eased}px),calc(-50% + ${dy*eased}px),0) rotate(${r*eased+dr*eased}deg) scale(${s*pop})`,'important')})}
function tick(){const label=activeLabel();const p=percent();if(label.includes('INTRO')){animateJar(ensureIntro(),p,'introScale','introY')}else if(label.includes('STRENGTH')){animateStrengthItems(ensureStrength(),p)}else if(label.includes('CATALOGUE')){animateCatalogueItems(ensureCatalogue(),p)}else{const visual=document.querySelector('.segment.is-active .visual');if(visual){const t=transformFor(p);const targetY=8-(18*p);motion.visualScale=spring(motion.visualScale,t.scale,.18);motion.visualY=spring(motion.visualY,targetY,.16);visual.style.setProperty('transform',`translateY(${motion.visualY}vh) scale(${motion.visualScale})`,'important')}}requestAnimationFrame(tick)}
requestAnimationFrame(tick);
